# FGSM Challenge — Scaffold Setup

Untargeted FGSM adversarial crafting against a hosted MNIST classifier. Files live in this directory.
...
Health check (expect `status`, `epsilon`, and `index`):

```bash
curl -s "${BASE_URL}/health" | jq
```

Challenge payload (note `label`, `epsilon`, and baseline `image_b64` in `[0,1]` pixel space after decode):
...
python3 -c "import torch; torch.load('fgsm_weights.pth', map_location='cpu'); print('weights ok')"
```

## 5) Run the scaffold

Why: Confirm imports, challenge fetch, model load, and orchestration wire up; execution should stop at the FGSM stub.

```bash
python3 fgsm_challenge.py --host "${BASE_URL}"
```

Expected behavior now:
- Challenge connects and weights download to `solver/fgsm_weights.pth` if missing.
- Local clean prediction prints.
- Execution stops when `fgsm_untargeted` is reached (stub) or on the immediate use of its return value.

## 6) After implementing the stub

Why: `/submit` checks `[0,1]` bounds, L∞ distance ≤ `epsilon`, and misclassification vs the baseline label before returning the flag.
...
On success, stdout should include adversarial `linf`/`pred` JSON and `Flag: HTB{...}`.

Manual predict/submit checks (optional):

```bash
# Expect 400 — clean image is not misclassified
...
- L∞ is measured in `[0,1]` space against the baseline from `/challenge`, not normalized tensors.
- PNG quantization can push L∞ slightly over `epsilon`; the scaffold retries with conservative effective epsilons after you implement the attack.
